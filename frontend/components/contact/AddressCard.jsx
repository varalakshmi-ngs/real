"use client";

import {
  Facebook,
  Instagram,
  Linkedin,
  MapPin,
  Mail,
  Phone,
  Twitter,
  Youtube,
} from "lucide-react";
import React from "react";
import { motion } from "framer-motion";

const AddressCard = () => {
  return (
    <section className="w-full bg-white py-16 px-6 sm:px-12 border-b border-gray-100 overflow-hidden">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col lg:flex-row gap-12 lg:gap-8 items-stretch">

          {/* CONTACT CARDS */}
          <div className="w-full lg:w-1/2 grid grid-cols-1 sm:grid-cols-2 gap-6">

            {/* LOCATION CARD */}
            <motion.div
              className="
                bg-gray-50
                p-8
                rounded-3xl
                border
                border-gray-100
                flex
                flex-col
                items-center
                sm:items-start
                text-center
                sm:text-left
                gap-4
                relative
                overflow-hidden
                group
                cursor-pointer
                transition-all
                duration-500
                hover:-translate-y-3
                hover:shadow-2xl
                hover:shadow-red-100
                hover:border-red-200
              "
              initial={{
                opacity: 0,
                y: 60,
                scale: 0.9,
              }}
              whileInView={{
                opacity: 1,
                y: 0,
                scale: 1,
              }}
              whileHover={{
                scale: 1.03,
                rotate: -1,
              }}
              viewport={{ once: true }}
              transition={{
                duration: 0.6,
                type: "spring",
                stiffness: 120,
              }}
            >

              {/* GLOW */}
              <div
                className="
                  absolute
                  -top-10
                  -right-10
                  w-32
                  h-32
                  bg-red-100
                  rounded-full
                  blur-3xl
                  opacity-0
                  group-hover:opacity-70
                  transition-all
                  duration-700
                "
              />

              <div
                className="
                  bg-white
                  p-4
                  rounded-2xl
                  shadow-sm
                  text-red-600
                  group-hover:bg-red-600
                  group-hover:text-white
                  group-hover:rotate-6
                  group-hover:scale-110
                  transition-all
                  duration-500
                "
              >
                <MapPin size={32} />
              </div>

              <div className="relative z-10">
                <h3 className="text-xl font-bold font-serif text-gray-900 mb-2">
                  Our Location
                </h3>

                <p className="text-gray-600 font-sans leading-relaxed">
                  123 Faith Avenue,
                  <br />
                  Spiritual City, SC 12345
                </p>
              </div>
            </motion.div>

            {/* EMAIL CARD */}
            <motion.div
              className="
                bg-gray-50
                p-8
                rounded-3xl
                border
                border-gray-100
                flex
                flex-col
                items-center
                sm:items-start
                text-center
                sm:text-left
                gap-4
                relative
                overflow-hidden
                group
                cursor-pointer
                transition-all
                duration-500
                hover:-translate-y-3
                hover:shadow-2xl
                hover:shadow-red-100
                hover:border-red-200
              "
              initial={{
                opacity: 0,
                y: 60,
                scale: 0.9,
              }}
              whileInView={{
                opacity: 1,
                y: 0,
                scale: 1,
              }}
              whileHover={{
                scale: 1.03,
                rotate: 1,
              }}
              viewport={{ once: true }}
              transition={{
                duration: 0.6,
                delay: 0.1,
                type: "spring",
                stiffness: 120,
              }}
            >

              {/* GLOW */}
              <div
                className="
                  absolute
                  -bottom-10
                  -left-10
                  w-32
                  h-32
                  bg-red-100
                  rounded-full
                  blur-3xl
                  opacity-0
                  group-hover:opacity-70
                  transition-all
                  duration-700
                "
              />

              <div
                className="
                  bg-white
                  p-4
                  rounded-2xl
                  shadow-sm
                  text-red-600
                  group-hover:bg-red-600
                  group-hover:text-white
                  group-hover:rotate-6
                  group-hover:scale-110
                  transition-all
                  duration-500
                "
              >
                <Mail size={32} />
              </div>

              <div className="relative z-10">
                <h3 className="text-xl font-bold font-serif text-gray-900 mb-2">
                  Email Us
                </h3>

                <a
                  href="mailto:info@realtemple.com"
                  className="text-gray-600 hover:text-red-600 transition-colors"
                >
                  info@realtemple.com
                </a>

                <p className="text-gray-500 text-sm mt-1">
                  We reply within 24 hrs
                </p>
              </div>
            </motion.div>

            {/* CALL CARD */}
            <motion.div
              className="
                bg-gray-50
                p-8
                rounded-3xl
                border
                border-gray-100
                flex
                flex-col
                items-center
                sm:items-start
                text-center
                sm:text-left
                gap-4
                relative
                overflow-hidden
                group
                cursor-pointer
                transition-all
                duration-500
                hover:-translate-y-3
                hover:shadow-2xl
                hover:shadow-red-100
                hover:border-red-200
                sm:col-span-2
                lg:col-span-1
              "
              initial={{
                opacity: 0,
                y: 60,
                scale: 0.9,
              }}
              whileInView={{
                opacity: 1,
                y: 0,
                scale: 1,
              }}
              whileHover={{
                scale: 1.03,
                rotate: -1,
              }}
              viewport={{ once: true }}
              transition={{
                duration: 0.6,
                delay: 0.2,
                type: "spring",
                stiffness: 120,
              }}
            >

              {/* GLOW */}
              <div
                className="
                  absolute
                  top-0
                  left-0
                  w-40
                  h-40
                  bg-red-100
                  rounded-full
                  blur-3xl
                  opacity-0
                  group-hover:opacity-70
                  transition-all
                  duration-700
                "
              />

              <div
                className="
                  bg-white
                  p-4
                  rounded-2xl
                  shadow-sm
                  text-red-600
                  group-hover:bg-red-600
                  group-hover:text-white
                  group-hover:rotate-6
                  group-hover:scale-110
                  transition-all
                  duration-500
                "
              >
                <Phone size={32} />
              </div>

              <div className="relative z-10">
                <h3 className="text-xl font-bold font-serif text-gray-900 mb-2">
                  Call Us
                </h3>

                <a
                  href="tel:+919898989898"
                  className="text-gray-700 font-medium text-lg hover:text-red-600 transition-colors"
                >
                  +91 98989 89898
                </a>

                <p className="text-gray-500 text-sm mt-1">
                  Mon-Fri: 9am - 5pm
                </p>
              </div>
            </motion.div>
          </div>

          {/* SOCIAL SECTION */}
          <motion.div
            className="
              w-full
              lg:w-1/2
              bg-red-600
              rounded-3xl
              p-8
              sm:p-12
              text-white
              flex
              flex-col
              justify-center
              relative
              overflow-hidden
              shadow-2xl
            "
            initial={{
              opacity: 0,
              x: 80,
              scale: 0.95,
            }}
            whileInView={{
              opacity: 1,
              x: 0,
              scale: 1,
            }}
            whileHover={{
              scale: 1.02,
            }}
            viewport={{ once: true }}
            transition={{
              duration: 0.8,
              type: "spring",
            }}
          >

            {/* BACKGROUND BLOBS */}
            <div className="absolute top-0 right-0 -mt-16 -mr-16 w-64 h-64 bg-red-500 rounded-full blur-3xl opacity-50 pointer-events-none" />

            <div className="absolute bottom-0 left-0 -mb-16 -ml-16 w-48 h-48 bg-red-700 rounded-full blur-3xl opacity-50 pointer-events-none" />

            <div className="relative z-10">
              <h2 className="text-3xl sm:text-5xl font-bold font-serif mb-5 leading-tight">
                Join Our Digital Community
              </h2>

              <p className="text-red-100 text-lg leading-relaxed mb-8 max-w-md">
                Stay updated with our latest sermons, events, and community
                news. Connect with us on social media for daily inspiration and
                fellowship.
              </p>

              <div className="flex flex-wrap gap-4">

                {[
                  <Facebook size={28} />,
                  <Twitter size={28} />,
                  <Instagram size={28} />,
                  <Linkedin size={28} />,
                  <Youtube size={28} />,
                ].map((icon, index) => (
                  <motion.a
                    key={index}
                    href="#"
                    whileHover={{
                      y: -8,
                      rotate: 6,
                      scale: 1.12,
                    }}
                    whileTap={{
                      scale: 0.95,
                    }}
                    className="
  bg-white
  text-red-600
  border-2
  border-white
  hover:bg-transparent
  hover:text-white
  p-4
  rounded-2xl
  transition-all
  duration-500
  backdrop-blur-sm
  shadow-sm
  hover:shadow-2xl
  hover:border-white
"
                  >
                    {icon}
                  </motion.a>
                ))}
              </div>
            </div>
          </motion.div>
        </div>
      </div>
    </section>
  );
};

export default AddressCard;