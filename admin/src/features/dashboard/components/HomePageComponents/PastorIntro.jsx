...
export default function PastorIntro({ data }) {
  ...
  const submitForm = async (formdata) => {
    ...
    try {
      ...
      const response = await apiRequest({
        method: "put",
        url: "/home/updatepastersection",
        data: formDataToSend,
      });

      if (response.success) {
        setSubmitStatus({ type: 'success', message: 'Pastor introduction updated successfully!' });
        setTimeout(() => setSubmitStatus(null), 3000);
      } else {
        setSubmitStatus({ type: 'error', message: 'Failed to update pastor introduction. Please try again.' });
      }
    } catch (error) {
  ...
  return (
    <OpenCloseLayout title="Pastor Introduction">
      <motion.form
      ...
      >
        ...
        >
          ...
          >
            ...
            {isSubmitting ? 'Updating...' : 'Update Pastor Intro'}
          </motion.button>
        </motion.div>
      </motion.form>
    </OpenCloseLayout>
